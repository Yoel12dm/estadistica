...
# 1.- Creación de un vector de color
color=c("green", "brown", "gold")

# 2.- Creación del gráfico
GB1<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "hotpink", fill=color)+
...
# 4.- Barras verticales
color=c("lightcyan1", "lightcyan3", "lightcyan4")

GB2<-ggplot(penguins, aes(x=año))+
  geom_bar(colour= "indianred", fill=color)+
  ggtitle("Gráfico de Barras")+
  xlab("Año")+
  ylab("Frecuencias")+
  coord_flip()+
...
HG<-ggplot(penguins, aes(x=largo_aleta_mm))+
  geom_histogram(col="green1", fill="green4")+
  ggtitle("Histograma")+
